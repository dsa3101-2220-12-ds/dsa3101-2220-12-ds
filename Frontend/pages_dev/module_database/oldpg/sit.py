    ...
    return dbc.Card([
        dbc.CardHeader(
            html.H5(
                dbc.Button(
                    title,
                    id=f"sit-main-option-{option_id}",
...
layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Div(id="sit-content", className="content-container"),
            dcc.Link(
...
def register_callbacks(app):
    @app.callback(
        ...
        [Input(f"sit-sub-option-{option_id}-{i}-{sub_option[0]}", "n_clicks") for option_id in range(1, len(categories) + 1) for i, sub_option in enumerate(category_options[categories[option_id - 1]])],
    )
    ...
    @app.callback(
        ...
        [Input(f"sit-main-option-{i}", "n_clicks") for i in range(1, len(categories) + 1)],
        [State(f"sit-sub-options-{i}", "is_open") for i in range(1, len(categories) + 1)],
    )
