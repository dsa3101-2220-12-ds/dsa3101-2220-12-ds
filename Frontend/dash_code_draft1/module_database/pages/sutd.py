...
def register_callbacks(app):
    @app.callback(
        ...
        [Input(f"sub-option-{option_id}-{i}-{sub_option[0]}", "n_clicks") for option_id in range(1, num_main_options + 1) for i, sub_option in enumerate(category_options[categories[option_id - 1]])],
    )
